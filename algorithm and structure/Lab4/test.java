...
class test {
    ...
    static void countSort(int a[], int n, int exp, int tab) {
        ...
        for (i = 0; i < n; i++) {
            System.out.println(sTab(tab + 1) + String.format("i < n (%d < %d => %b)", i, n, i < n));
        ...
        for (i = 1; i < 10; i++) {
            System.out.println(sTab(tab + 1) + String.format("i < 10 (%d < %d => %b)", i, 10, i < 10));
        ...
        for (i = n - 1; i >= 0; i--) {
            System.out.println(sTab(tab + 1) + String.format("i >= 0 (%d >= %d => %b)", i, 0, i >= 0));
    ...
    static void radixsort(int a[], int n, int tab) {
        System.out.println(sTab(tab) + "a: " + Arrays.toString(a));
        int m = getMax(a, n);
        System.out.println(sTab(tab) + String.format("m = getMax(a, n) = %d", getMax(a, n)));

        System.out.println(sTab(tab) + String.format("exp = 1"));
        int exp;
        for (exp = 1; m / exp > 0; exp *= 10) {
            System.out.println(sTab(tab + 1) + String.format("m / exp > 0 (%d / %d > 0 => %b)", m, exp, m / exp > 0));
